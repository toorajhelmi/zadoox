...
import {
  buildConceptionChatPrompt,
  buildConceptionExtractIgPrompt,
  buildConceptionKpExtractorPrompt,
  buildConceptionSimUserPrompt,
  buildConceptionTwoStageDmPrompt,
  ExtractIGSchema,
  KpExtractorSchema,
  SimUserSchema,
  TwoStageDmSchema,
} from '../../services/ai/conception/prompts.js';
import { enforceContextGroupNoAnchorChildEdges } from '../../services/ai/conception/context-group.js';
  ...
  fastify.post(
    ...
    async (request: AuthenticatedRequest, reply) => {
      try {
        ...
        const p = buildConceptionChatPrompt({ dr: body.dr, action: body.action, message });
        const raw = await service.chatJson({ system: p.system, user: p.user, temperature: 0.35 }, body.model);
  ...
  fastify.post(
    ...
    async (request: AuthenticatedRequest, reply) => {
      try {
        ...
        const service = getAIService();
        const p = buildConceptionExtractIgPrompt({ dr: body.dr, message });
        const raw = await service.chatJson({ system: p.system, user: p.user, temperature: 0.2 }, body.model);
        const parsed = ExtractIGSchema.parse(raw);
  ...
  fastify.post(
    ...
    async (request: AuthenticatedRequest, reply) => {
      try {
        ...
        const dmPrompt = buildConceptionTwoStageDmPrompt({ dr: body.dr, message });
        const dmRaw = await service.chatJson({ system: dmPrompt.system, user: dmPrompt.user, temperature: 0.35 }, body.model);
        const dm = TwoStageDmSchema.parse(dmRaw);
        ...
        const kpPrompt = buildConceptionKpExtractorPrompt({
          ideaGraphLabels: existingLabels,
          uiPinnedKps,
          contextGroup: isRecord(drAny.contextGroup) ? drAny.contextGroup : null,
          turns: kpTurns,
        });

        const kpRaw = await service.chatJson({ system: kpPrompt.system, user: kpPrompt.user, temperature: 0.25 }, body.model);
  ...
  fastify.post(
    ...
    async (request: AuthenticatedRequest, reply) => {
      try {
        ...
        const service = getAIService();
        const p = buildConceptionSimUserPrompt({ dr: body.dr });
        const raw = await service.chatJson({ system: p.system, user: p.user, temperature: 0.6 }, body.model);
        const parsed = SimUserSchema.parse(raw);
