...
function mergeTwoStageKps(
  ...
  for (const raw of edges) {
    ...
    const src = byNorm.get(normalizeLabel(srcLabel) + ctxKeyForFacets(edgeFacets))?.id ?? byNorm.get(normalizeLabel(srcLabel))?.id ?? null;
    const dst = byNorm.get(normalizeLabel(dstLabel) + ctxKeyForFacets(edgeFacets))?.id ?? byNorm.get(normalizeLabel(dstLabel))?.id ?? null;
    if (!src || !dst || src === dst) continue;
  ...
  try {
    ...
    for (const g of groupsForTurn) {
      ...
      for (const aid of anchorIds) {
        const exists = (ig.edges ?? []).some((e) => e.src === aid && e.dst === groupNode.id && Array.isArray(e.facets) && e.facets.includes('REL:anchors'));
        if (exists) continue;
        ig.edges.push({
          src: aid,
          dst: groupNode.id,
      ...
      for (const cid of Array.from(ctxNodeIds)) {
        const exists = (ig.edges ?? []).some((e) => e.src === groupNode.id && e.dst === cid && Array.isArray(e.facets) && e.facets.includes('REL:in_group'));
        if (exists) continue;
        ig.edges.push({
          src: groupNode.id,
          dst: cid,
      ...
      if (anchorIds.length > 0) {
        ig.edges = (ig.edges ?? []).filter((e) => {
          ...
          if (e.dst === groupNode.id) return true;
